...
int main() {
    struct Zona zonas[MAX_LOCATIONS] = {
        {"Tumbaco", 7, 215, 3, 11},
    ...
    do {
        ...
        switch (opcion) {
            case 1:
                printf("\nNiveles de contaminacion en las distintas localidades ultimos 30 dias\n");
                for (int i = 0; i < totalZonas; i++) {
                    printf("\nZona: %s\nPM2.5: %.2f ug/m3\nCO: %.2f ppb\nSO2: %.2f ppb\nNO2: %.2f ppb\n",
                           zonas[i].nombre, zonas[i].pm25, zonas[i].co, zonas[i].so2, zonas[i].no2);
                    evaluarContaminacion(&zonas[i]);
                    printf("Prediccion: %s\n", predecirCambio(&zonas[i]));
                }
                break;

            case 2:
                if (totalZonas < MAX_LOCATIONS) {
